...
// GET horário do professor (AGORA BUSCA NAS ATRIBUICOES)
app.get('/meu-horario/:id_professor', async (req, res) => {
    ...
    try {
        ...
        // Formatar para o frontend esperar o que já usava (opcional, mas ajuda a manter compatibilidade)
...
app.post('/atribuicoes', async (req, res) => {
    ...
    try {
        ...
        if (conflitos.length > 0) {
            return res.status(409).json({ error: `A sala ${sala} já está ocupada neste horário!` });
        }
        ...
        const id_horario = hResult.insertId;

        await db.execute('INSERT INTO atribuicoes (id_professor, id_disciplina, id_turma, id_horario, sala, descricao) VALUES (?, ?, ?, ?, ?, ?)', [id_professor, id_disciplina, id_turma, id_horario, sala, descricao]);
...
app.put('/atribuicoes/:id', async (req, res) => {
    ...
    try {
        ...
        if (conflitos.length > 0) {
            return res.status(409).json({ error: `Conflito: A sala ${sala} já está ocupada neste horário!` });
        }
...
app.post('/admin/professores', async (req, res) => {
    ...
    try {
        await db.execute('INSERT INTO professores (nome, email, disciplinas_permitidas) VALUES (?, ?, ?)', [nome, email, disciplinas_permitidas]);
...
app.put('/admin/professores/:id', async (req, res) => {
    ...
    const { nome, email, disciplinas_permitidas } = req.body;
    try {
        await db.execute('UPDATE professores SET nome = ?, email = ?, disciplinas_permitidas = ? WHERE id_professor = ?', [nome, email, disciplinas_permitidas, id]);
